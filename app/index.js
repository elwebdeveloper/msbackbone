...
var BackboneGenerator = yeoman.generators.Base.extend({
  constructor: function () {
    ...
    this.env.options.appPath = this.options.appPath || 'web';
  ...
  prompting: function () {
    ...
    /* var prompts2 = [{
      ...
      choices: ['No', 'Foundation', 'Bootstrap'],
      filter: function (val) { return val.toLowerCase(); }
    }, {
      type: 'list',
      name: 'modularizing',
      message: 'No, AMD or CommomJS?',
      choices: ['No', 'Require', 'Browserify'],
      filter: function (val) { return val.toLowerCase(); }
    }, {
      type: 'list',
      name: 'Backbone',
      message: 'Backbone or Marionette?',
      choices: ['No', 'Backbone', 'Marionette'],
      filter: function (val) { return val.toLowerCase(); }
    }]; */
  ...
  writing: {
    ...
    mainStylesheet: function () {
      ...
      if (this.compassBootstrap) {
        this.template('main.scss', this.env.options.appPath + '/css/main.scss');
        return;
      }
    ...
    writeIndexWithRequirejs: function () {
      ...
      /* The following has been commented out as an update in the grunt-usemin prevent the below code to work properly
       * this.indexFile = this.appendScripts(this.indexFile, 'js/main.js', [
        'bower_components/requirejs/require.js'
      ], {'data-main': 'js/main'}); */

      this.indexFile = this.appendScripts(this.indexFile, 'js/main.js', [
        'bower_components/requirejs/require.js', 'js/main.js'
      ]);
    },
    ...
    setupEnv: function () {
      ...
      this.mkdir(this.env.options.appPath + '/js/vendor/');
      this.mkdir(this.env.options.appPath + '/css');
      this.mkdir(this.env.options.appPath + '/images');
      this.copy('web/404.html', this.env.options.appPath + '/404.html');
      this.copy('web/favicon.ico', this.env.options.appPath + '/favicon.ico');
      this.copy('web/robots.txt', this.env.options.appPath + '/robots.txt');
    ...
    createRequireJsAppFile: function () {
      ...
      this._writeTemplate('requirejs_app', this.env.options.appPath + '/js/main');
    },
    ...
    createAppFile: function () {
      ...
      this._writeTemplate('app', this.env.options.appPath + '/js/main');
    },

    composeTest: function () {
      // console.log(this.options);
      if (['msbackbone:app', 'backbone'].indexOf(this.options.namespace) >= 0) {
